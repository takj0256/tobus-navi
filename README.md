...
作業前に [AGENTS.md](AGENTS.md) と [現在の状態](docs/STATUS.md) を読み、Gitの更新を確認してください。
[構成と前提](docs/PROJECT_CONTEXT.md)、[判断の記録](docs/DECISIONS.md)、[運用手順](docs/OPERATIONS.md) で会話の決定事項を共有します。
コードの同期と本番・バッチへの反映は別操作です。未実装・未確認事項も STATUS.md に残します。
...
- 路線マップの経路切り替えに始点・行き先・停留所数を表示
- 路線マップへ運行中のバスを停留所イベントから推定配置し、10秒ごとに更新
- 地図上のバスマーカーにも水素FC・EV・ハイブリッドの種別を反映
- 同じ行き先へ合流する経路は、最長経路を赤い本線、合流前の枝線を別色で同時表示
- 本線途中から始まる区間便は重複線を追加せず、同じ本線上の便として表示
- Service Workerキャッシュを `tobus-navi-v19` へ更新
...
Phase 6以降で生成済みの次のデータをそのまま使用できます。`shapes.txt` を取り込んだデータでは公式走行経路を表示し、shapeがない便だけ停留所間の概略表示になります。
