...
- 毎分Cron：停留所イベント収集、異常判定、R2時間圧縮
- 15分ごと：Open-Meteoから東京中心点の気温・降水・降雪を取得
- 毎分：完了済みの分イベントを古い時間から回収し、CPU上限対策として1時間ずつ時間バッチへ統合
- 毎日04:15：サブPCが前日 `daily-v2` の欠損を時間バッチから生成し、28日統計をD1へ反映
...
非対話で成功することを確認してください。前日分の `daily-v2` がない場合はサブPC上で
時間バッチから生成してから集計します。生成不能または集計失敗時はD1の `job_status` を
`failed` へ更新し、古い `complete` を本日の成功と誤認しません。
